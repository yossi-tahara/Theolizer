...
enum型をシリアライズする際に何を記録するのか選択できます。
...
// ***************************************************************************
全ての変数についてオブジェクト追跡すれば確実にアドレス解決できるのですが、ポイントされていないものまで追跡するのは無駄です。<br>
そこで、下記のように追跡するインスタンスに追跡指定するようにしました。
...
2. アップデートで可能なenum型の定義変更<br>
    ...
    シンボルの削除<br>
    シンボルの追加<br>
    ただし、新しいプログラムが保存したデータを古いプログラムが回復することは原則としてできません。<br>
    古いプログラムは新しいプログラムで追加／削除／変更されたシンボルとの対応を把握していないためです。
<br>
...
グローバル・バージョン番号は、一つ以上のローカル・バージョン番号を上げた時にインクリメントして下さい。回復対象のシリアライズ・データ内の全ローカル・バージョン番号を特定できるように上げればOKです。<br>Theolizerドライバは、グローバル・バージョン番号に対応する全てのローカル・バージョン番号を「グローバル・バージョン番号テーブル」へ自動的に生成します。<br>

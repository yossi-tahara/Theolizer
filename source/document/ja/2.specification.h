...
- **非侵入型完全自動**<br>
クラス定義を変更しないまま、自動的にシリアライズ処理するクラスです。<br>
...
そこで、原則としてポインタ経由でシリアライズされた「型」についてオブジェクト追跡しています。ただし、int型等のプリミティブについて1つのint型変数がポイントされただけで全てのint型を追跡するのは無駄が多いです。そこで、プリミティブについては追跡する変数自体に追跡指定するようにしました。

そこで、下記についてオブジェクト追跡を行ようにしました。

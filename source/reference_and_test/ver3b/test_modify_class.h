...
//----------------------------------------------------------------------------
    ...
    void check()
    {
        ...
        for (unsigned j=0; j < kSize1; ++j)
        {
            for (unsigned k=0; k < kSize0; ++k)
            {
                if ((gVersionList[gDataIndex].mVersionEnum == VersionEnum::ver3b)
                 && (gVersionList[gProgramIndex].mVersionEnum == VersionEnum::ver3b))
                {
                        THEOLIZER_EQUAL(mArrayDim[j][k], j*10+k, j, k);
                }
                else if ((VersionEnum::ver1c <= gVersionList[gDataIndex].mVersionEnum)
                      && (gVersionList[gDataIndex].mVersionEnum <= VersionEnum::ver3b))
                {
